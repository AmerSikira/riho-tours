...
class UpdateAranzmanRequest extends FormRequest
{
    /**
    ...
    {
        $aranzmanId = $this->route('aranzman')?->id;
        ...
        return [
            'sifra' => [
                ...
                'string',
                'max:100',
                Rule::unique('arrangements', 'sifra')->ignore($aranzmanId),
            ],
            ...
            'zadrzane_slike' => ['nullable', 'array'],
            'zadrzane_slike.*' => ['string'],
            'nove_slike' => ['nullable', 'array'],
            'nove_slike.*' => ['image', 'max:5120'],
            'main_image_selection' => ['nullable', 'regex:/^(existing:[A-Za-z0-9-]+|new:\d+)$/'],
        ];
    }
    ...
    public function messages(): array
    {
        return [
            ...
            'nove_slike.*.image' => 'Svaka nova datoteka mora biti validna slika.',
            'nove_slike.*.max' => 'Slika može imati maksimalno 5MB.',
        ];
    }
}
